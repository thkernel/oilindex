...
class StatementsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create statement" do
    assert_difference('Statement.count') do
      post statements_url, params: { statement: { product_id: @statement.product_id, statement_time: @statement.statement_time, statement_type_id: @statement.statement_type_id, status: @statement.status, user_id: @statement.user_id } }
    end
  ...
  test "should update statement" do
    patch statement_url(@statement), params: { statement: { product_id: @statement.product_id, statement_time: @statement.statement_time, statement_type_id: @statement.statement_type_id, status: @statement.status, user_id: @statement.user_id } }
